...
from models import Genre, venue_genres, artist_genres, Venue, Artist, Show
#----------------------------------------------------------------------------#
# App Config.
#----------------------------------------------------------------------------#
...
def venues():
  ...
  try:
    ...
    for area in areas:
      list_of_venues = Venue.query.filter(Venue.state==area.state).filter(Venue.city==area.city).all()
      venue_arr = []
      ...
      for venue in list_of_venues:
        venue_obj = {
          "id": venue.id,
          "name": venue.name,
          "num_upcoming_shows": len(db.session.query(Show).filter_by(venue_id=venue.id).filter(Show.time_of_show > datetime.now()).all()),
        }
...
def search_venues():
  ...
  try:
    ...
    search_results = db.session.query(Venue).filter(Venue.name.ilike("%{}%".format(search_term.replace(" ", "\ ")))).all()
...
def search_artists():
  ...
  try:
    search_term = request.form.get('search_term')
    search_results = db.session.query(Artist).filter(Artist.name.ilike("%{}%".format(search_term.replace(" ", "\ ")))).all()
